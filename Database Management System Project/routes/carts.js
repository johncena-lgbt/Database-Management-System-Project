const express = require('express');
const cartController = require('../controllers/cartsController');
const jwtMiddleware = require('../middleware/jwtMiddleware');

const router = express.Router();

// All routes in this file will use the jwtMiddleware to verify the token and check if the user is an admin.
// Here the jwtMiddleware is applied at the router level to apply to all routes in this file
// But you can also apply the jwtMiddleware to individual routes
// router.use(jwtMiddleware.verifyToken, jwtMiddleware.verifyIsAdmin);

router.use(jwtMiddleware.verifyToken);

// To create a cart and to put products in the cart
router.post('/', cartController.createCart);

// To get all cart items
router.get('/', cartController.getAllCartItems)

// To update cart item quantity
router.put('/', cartController.updateCartItem);

// To update cart item quantity in bulk
router.put('/bulk', cartController.updateMultipleCartItems);

// To delete a cart item
router.delete('/', cartController.deleteSingleCartItem);

// To cart items in bulk
router.delete('/bulk', cartController.deleteMultipleCartItems);

// To get cart summary
router.get('/summary', cartController.getCartSummary);

// Cart Checkout
router.post('/checkout', cartController.checkout);

module.exports = router;